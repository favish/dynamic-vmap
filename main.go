		...
		AdBreaks: []vmap.AdBreak{
			...
			},
			{
				...
				AdSource: &vmap.AdSource{
					...
					AdTagURI: &vmap.AdTagURI{
						TemplateType: "vast3",
						URI:          "https://pubads.g.doubleclick.net/gampad/ads?iu=/21841313772/" + partnerUnitCodes[2] + "&env=vp&impl=s&correlator=&tfcd=0&npa=0&gdfp_req=1&output=vast&sz=640x480&unviewed_position_start=1&description_url=" + descriptionUrl  + "&vpos=postroll&cust_params=testing%3Dtrue",
					},
					CustomAdData: nil,
				},
				TrackingEvents: nil,
...
func adBreakGenerator(offset vast.Duration, descriptionUrl string, breakId string, minSec int, maxSec int, maxPods string, adUnits[3]string, pod float64, vpos string) vmap.AdBreak {
	...
	return vmap.AdBreak{
		...
		AdSource: &vmap.AdSource{
			...
			AdTagURI: &vmap.AdTagURI{
				TemplateType: "vast3",
				URI: fmt.Sprintf("https://pubads.g.doubleclick.net/gampad/ads?iu=/21841313772/" + adUnits[1] + "&env=vp&impl=s&correlator=&tfcd=0&npa=0&gdfp_req=1&output=vast&sz=640x480&unviewed_position_start=1&description_url=%s&pmnd=%v&pmxd=%v&pmad=%v&pod=%v&vpos=%v&cust_params=testing%3Dtrue",
					descriptionUrl,
					minSeconds,
					maxSeconds,
					maxPods,
