...
func createVmap(w http.ResponseWriter, r *http.Request) {
    ...
    adGapSeconds := 600.0 // An ad pod every 10 minutes.
		...
		AdBreaks: []vmap.AdBreak{
			{
				...
				AdSource: &vmap.AdSource{
					...
					AdTagURI: &vmap.AdTagURI{
						TemplateType: "vast3",
						URI:          "https://pubads.g.doubleclick.net/gampad/ads?iu=/21841313772/" + partnerUnitCodes[0] + "&env=vp&impl=s&correlator=&tfcd=0&npa=0&gdfp_req=1&output=vast&sz=640x480&unviewed_position_start=1&max_ad_duration=15000&description_url=" + descriptionUrl + "&vpos=preroll&vad_type=linear&pp=realvision",
					},
					CustomAdData: nil,
				},
				TrackingEvents: nil,
